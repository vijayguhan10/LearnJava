...
# Vehicle Parking Management – SOLID Design (Java)
...
* `Vehicle` → stores vehicle-related data only
* `ParkingFeeCalculator` → calculates parking fees
* `TicketGeneratingService` → generates ticket IDs
* `UpiPayment` / `CreditCardPayment` → handles payment logic
* `VehicleService` → orchestrates vehicle processing
